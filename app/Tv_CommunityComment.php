...
//		$hash = md5($id);
	...
	function deleteImage()
	{
		...
		if($this->get('image_id')) {
			...
			$im->remove($this->get('image_id'),'community_comment',$this->get('community_comment_id'));
	...
	function deleteMovie()
	{
		...
		if($this->get('movie_id')) {
			...
			$im->remove($this->get('movie_id'),'community_comment',$this->get('community_comment_id'));
			// 動画を論理削除
			$this->set('movie_id', 0);
		}
		$this->update();
