...
class Tv_Action_AdminDecomailEditDo extends Tv_ActionAdminOnly
{
	...
	function perform()
	{
		...
		// デコメールファイル1のアップロード(1=編集)
		if($this->af->get('decomail_file1_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('decomail_file1', $um->uploadFile($this->af->get('decomail_file1_file'),'decomail'));
		}
		// デコメールファイル1の削除(2=削除)
		elseif($this->af->get('decomail_file1_status') == 2){
			$o->set('decomail_file1', '');
		}
		// デコメールファイル2のアップロード(1=編集)
		if($this->af->get('decomail_file2_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('decomail_file2', $um->uploadFile($this->af->get('decomail_file2_file'),'decomail'));
		}
		// デコメールファイル2の削除(2=削除)
		elseif($this->af->get('decomail_file2_status') == 2){
			$o->set('decomail_file2', '');
		}
		// DOCOMO用デコメールファイルのアップロード(1=編集)
		if($this->af->get('decomail_file_d_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('decomail_file_d', $um->uploadFile($this->af->get('decomail_file_d_file'),'decomail'));
		}
		// DOCOMO用デコメールファイルの削除(2=削除)
		elseif($this->af->get('decomail_file_d_status') == 2){
			$o->set('decomail_file_d', '');
		}
		// AU用デコメールファイル_aのアップロード(1=編集)
		if($this->af->get('decomail_file_a_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('decomail_file_a', $um->uploadFile($this->af->get('decomail_file_a_file'),'decomail'));
		}
		// AU用デコメールファイル_aの削除(2=削除)
		elseif($this->af->get('decomail_file_a_status') == 2){
			$o->set('decomail_file_a', '');
		}
		// SOFTBANK用デコメールファイル_sのアップロード(1=編集)
		if($this->af->get('decomail_file_s_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('decomail_file_s', $um->uploadFile($this->af->get('decomail_file_s_file'),'decomail'));
		}
		// SOFTBANK用デコメールファイル_sの削除(1=削除)
		elseif($this->af->get('decomail_file_s_status') == 2){
