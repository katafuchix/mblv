...
/**
 * 管理送料編集実行アクションフォームクラス
 * 
 * @author     Technovarth
 * @access     public
 * @package    MBLV
 */
class Tv_Form_AdminEcPostageEditDo extends Tv_ActionForm
{
 ...
 * @package    MBLV
 */
class Tv_Action_AdminEcPostageEditDo extends Tv_ActionAdminOnly
{
	/**
	...
	function prepare()
	{
		if ($this->af->validate() > 0) return 'admin_ec_postage_edit';
		...
		$param = array(
			'sql_select'	=> '*',
			'sql_from'		=> 'postage',
			'sql_where'		=> 'postage_id <> ? AND postage_name = ?',
			'sql_values'	=> array($this->af->get('postage_id'),$this->af->get('postage_name')),
		);
	...
	{
		// 情報更新準備
		$postageObject =& new Tv_Postage($this->backend,'postage_id',$this->af->get('postage_id'));
		$postageObject->importForm(OBJECT_IMPORT_IGNORE_NULL);
		$postageObject->set('postage_same_status', $this->af->get('postage_same_status'));
		
		$timestamp = date('Y-m-d H:i:s');
		$postageObject->set('postage_updated_time', $timestamp);
		
		// 情報更新
		$r = $postageObject->update();
		if(Ethna::isError($r)) return 'master_postage_edit';
		
		$this->af->clearFormVars();
		
		$this->ae->add(null, '', I_ADMIN_POSTAGE_EDIT_DONE);
