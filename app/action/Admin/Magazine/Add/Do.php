...
class Tv_Action_AdminMagazineAddDo extends Tv_ActionAdminOnly
{
	...
	function prepare()
	{
		...
		// HTMLメールチェックがなくテキスト本文もない場合 → HTML メール送信と判断する
		if(!$this->af->get('magazine_body_html_status') && !$this->af->get('magazine_body_text')){
				$this->ae->add('errors', "「HTMLメールを使用する」にチェックを入れてください。");
	...
	function perform()
	{
		...
		$this->ae->add(null, '', I_ADMIN_MAGAZINE_ADD_DONE);
