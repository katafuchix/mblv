...
/**
 * 管理セグメント登録実行処理アクションフォームクラス
 * 
 * @author     Technovarth
 * @access     public
 * @package    MBLV
 */
class Tv_Form_AdminSegmentAddDo extends Tv_ActionForm
{
...
/**
 * 管理セグメント登録実行処理アクション実行クラス
 * 
 * @author     Technovarth
 * @access     public
 * @package    MBLV
 */
class Tv_Action_AdminSegmentAddDo extends Tv_ActionAdminOnly
{
	/**
	...
	{
		// 検証エラーの場合
		if ($this->af->validate() > 0) return 'admin_segment_add';
		return null;
	}
	...
	{
		// セグメント登録
		$m =& new Tv_Segment($this->backend);
		$m->importForm(OBJECT_IMPORT_IGNORE_NULL);
		$m->set('segment_status',1);
		// セグメントをセット
		...
		$r = $m->add();
		// 登録エラーの場合
		if(Ethna::isError($r)) return 'admin_segment_add';
		
		$this->ae->add(null, '', I_ADMIN_SEGMENT_ADD_DONE);
