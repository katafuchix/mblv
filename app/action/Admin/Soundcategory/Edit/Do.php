...
class Tv_Action_AdminSoundcategoryEditDo extends Tv_ActionAdminOnly
{
	...
	function perform()
	{
		...
		// サウンドカテゴリファイル1のアップロード(1=編集)
		if($this->af->get('sound_category_file1_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('sound_category_file1', $um->uploadFile($this->af->get('sound_category_file1_file'),'sound'));
		}
		// サウンドカテゴリファイル1の削除(2=削除)
		elseif($this->af->get('sound_category_file1_status') == 2){
			$o->set('sound_category_file1', '');
		}
		// サウンドカテゴリファイル2のアップロード(1=編集)
		if($this->af->get('sound_category_file2_status') == 1){
			$um =& $this->backend->getManager('Util');
			$o->set('sound_category_file2', $um->uploadFile($this->af->get('sound_category_file2_file'),'sound'));
		}
		// サウンドカテゴリファイル2の削除(2=削除)
		elseif($this->af->get('sound_category_file2_status') == 2){
