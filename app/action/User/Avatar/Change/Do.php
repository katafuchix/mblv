...
class Tv_Action_UserAvatarChangeDo extends Tv_ActionUserOnly
{
	...
	function perform()
	{
		...
		if($ua->get('user_avatar_wear')==1 && $this->af->get('cmd') == 'off'){
			...
			// もし、アバターシステムカテゴリが、「1.顔」「2.体」「3.髪」「4.服トップス」「5.服ボトムス」「6.服ワンピース」「8.履物」「13.背景」のものを着衣した場合、ユーザアバターにある、左記アバターシステムカテゴリのアバターを脱衣する
			...
			if($a->isValid()){
				...
				// アバターシステムカテゴリに該当を検索
				if($ac->isValid()){
					if(in_array($ac->get('avatar_system_category_id'), array(1,2,3,4,5,6,8,13))){
		...
		// アバターのキャッシュ
		$avatar = $this->backend->getManager('Avatar');
		$this->af->set('user_id', $user_id);
		foreach(array('i', 't', 's') as $v){
			$this->af->set('attr', $v);
			$avatar->preview(true);
		}
