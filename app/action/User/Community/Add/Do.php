...
class Tv_Action_UserCommunityAddDo extends Tv_ActionUserOnly
{
	...
	function perform()
	{
		...
		$community_member =& new Tv_CommunityMember($this->backend);
		$community_member->set('community_id', $community->getId());
		$community_member->set('user_id', $user['user_id']);
		$community_member->set('community_member_status', 2);
		$community_member->add();
