 ...
 * @package    MBLV
 */
class Tv_Action_UserFileAvatarView extends Tv_ActionUserOnly
{
	/**
	...
	function perform()
	{
		...
		// サイズの決定
		if($this->af->get('width')) $avatar->setWidth($this->af->get('width'));
		if($this->af->get('height')) $avatar->setHeight($this->af->get('height'));
		// レイヤーの追加
		foreach($item as $k => $v){
			if($v) $avatar->addLayer($this->config->get('file_path') . 'avatar/' . $v);
		}
		// 出力
		$avatar->build();
		exit;
