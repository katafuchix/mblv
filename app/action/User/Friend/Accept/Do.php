	...
	function perform()
	{
		...
		if($this->af->get('accept')){
			...
			$param = array(
				...
				'sql_from'		=> ' invite ',
				'sql_where'		=> ' from_user_id = ? AND to_user_id = ? AND invite_status = ?',
				'sql_values'	=> array( $from_user_id, $user_id, 1 ),
			);
			$list = $um->dataQuery($param);
			
			//あるなら更新
			if(count($list) > 0){
				//$o =& new Tv_Invite($this->backend,'invite_id',$list[1][0]['invite_id']);
				$o =& new Tv_Invite($this->backend,'invite_id',$list[0]['invite_id']);
