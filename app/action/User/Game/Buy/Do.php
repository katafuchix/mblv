...
class Tv_Action_UserGameBuyDo extends Tv_ActionUserOnly
{
	...
	function perform()
	{
		...
		// 配信終了日時を超えているの場合
		 ============================================= */
		if ( $g->get('game_end_status') == 1 ) {
			if ( $g->get('game_end_time') <= date('Y-m-d H:i:s') ) {
				$this->ae->add(null, '', E_USER_GAME_OUT_OF_DATE);
				return 'user_error';
			}
		}
		...
		$param = array(
			'sql_select'	=> ' game_id ',
			'sql_from'		=> ' user_game ',
			'sql_where'		=> 'user_id = ? AND game_id  = ? AND user_game_status <> ?',
			'sql_values'	=> array( $user['user_id'], $game_id, 0 ),
		);
		$ug_list = $um->dataQuery($param);
