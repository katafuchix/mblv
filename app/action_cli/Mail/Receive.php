...
class Tv_Cli_Action_MailReceive extends Tv_ActionUser
{
	...
	function regist_mail()
	{
		...
		// 新規のユーザの場合
			...
			// 過去に登録した履歴があるユーザの場合
			$u =& new Tv_User($this->backend, 'user_mailaddress', $this->mail_from);
			if($u->isValid()){
				// 一旦物理削除する
				$r = $u->remove();
	...
	function my_image_mail()
	{
		...
		if($fileId || !Ethna::isError($r)){
			...
			$this->ms->sendOne($this->mail_from , 'user_image_success_error' , $value );
		}
