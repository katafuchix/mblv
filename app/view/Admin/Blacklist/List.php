	...
	function preforward()
	{
		...
		$condition = array(
			...
			'black_list_created' => array(
				'type' => 'PERIOD',
				'column' => 'bl.black_list_created_time',
			),
			// 更新日時検索
			'black_list_updated' => array(
				'type' => 'PERIOD',
				'column' => 'bl.black_list_updated_time',
			),
			// 削除日時検索
			'black_list_deleted' => array(
				'type' => 'PERIOD',
				'column' => 'bl.black_list_deleted_time',
			),
		);
		
		// 初期画面ではステータスが「有効」のデータのみ取得する
		if($this->af->get('black_list_status') == "") $this->af->set('black_list_status', 1);
		...
		// ページャ
		$this->listview = array(
			'sql_select'	=> 'fu.user_id as black_list_user_id,fu.user_nickname as black_list_user_nickname,' .
				'tu.user_id as black_list_deny_user_id,tu.user_nickname as black_list_deny_user_nickname,' .
				'bl.black_list_id,bl.black_list_status,bl.black_list_checked,bl.black_list_created_time,bl.black_list_updated_time,bl.black_list_deleted_time',
			'sql_from'		=> 'user as fu,user as tu,black_list as bl',
			'sql_where'		=> $sqlWhere,
			'sql_order'		=> 'bl.black_list_updated_time DESC',
