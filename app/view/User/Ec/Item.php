 ...
 * @package    MBLV
 */
class Tv_View_UserEcItem extends Tv_ListViewClass
{
	/**
	 *  画面表示前処理
	 *
	...
	function preforward()
	{
		...
		$this->af->setAppNE('item_text1',$itemObject->get('item_text1'));
		...
		// カテゴリフリースペースコンテンツを生成
		$this->af->setAppNe('item_contents_body', $um->convertHtml($itemObject->get('item_contents_body')));
		...
		// 個数オプション
		for($i=1;$i<=9;$i++){
			$cart_item_num_list[$i]['name'] = $i;
		}
		$this->af->setApp("cart_item_num_list",$cart_item_num_list);
