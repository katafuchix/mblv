...
class Tv_CommunityMemberManager extends Ethna_AppManager
{
	...
	// コミュニティーメンバーのステータスを削除にする
	function status_off($communityMemberId)
	{
		// コミュニティメンバー情報を取得
		$communityMember =& new Tv_CommunityMember(
			$this->backend,
		...
		// 該当コミュニティのメンバー数減少
		if($communityMember->get('community_member_status') != 0){
			$community = &new Tv_Community($this->backend,'community_id',$communityMember->get('community_id'));
			// 有効なレコードの場合
			if($community->isValid()){
				$community_member_num = $community->get('community_member_num') - 1;
				$community->set('community_member_num', $community_member_num);
				$community->update();
			}
		}
