...
class Tv_MovieManager extends Ethna_AppManager
{
	 ...
	 */
	function remove($movieId, $movieStatus = 0)
	{
		// ファイルオブジェクトを取得する
		...
		// DB更新
		$movie->set('movie_status', $movieStatus);
	...
	function convert($param)
	{
		...
		if( !is_movie($det) ){
			$com = $this->config->get('ffmpeg_path') . " -i {$movie_path1} -an -f flv {$movie_path2}"; //-an
			system( $com );
			$com = $this->config->get('flvtool2_path') . " -U {$movie_path2}";
			system( $com );
		}
//		// パーミッションの変更
