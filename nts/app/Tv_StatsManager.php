...
//}
		// 個別に集計処理を必要する場合は行う
		if(in_array($type, array('ad','banner','media')) && in_array($term ,array('hour','day'))){
			// 準備
	...
	function getStatsType()
	{
		...
		$stats_type = array(
			...
			'media'	=> array(
				...
				'sql_score'		=> array('access','mail','regist','send','resign'),
				'score_status'	=> array(0,1,2,3,4),
				'sql_order'		=> 'access DESC',
				'sql_select'	=> array(
					'hour' => array(
//							'media_id as id',
//							'media_access_status as status', 
							'id',
							'status', 
				  	),
					'day' => array(
...
//							'media_access_status as status', 
			...
			'login'	=> array(
				...
				'sql_score'		=> array('login'),
				'score_status'	=> array(0),
				'sql_order'		=> 'login DESC',
				'sql_select'	=> array(
					'hour'		=> array(
							'id',
							'COUNT( * ) AS login',
