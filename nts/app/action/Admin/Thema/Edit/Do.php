...
/**
 * 管理お題編集実行処理アクションフォームクラス
 * 
 * @author Technovarth
 * @access public
 * @package SNSV
 */
class Tv_Form_AdminThemaEditDo extends Tv_ActionForm
{
...
/**
 * 管理お題編集実行処理アクション実行クラス
 * 
 * @author Technovarth
 * @access public
 * @package SNSV
 */
class Tv_Action_AdminThemaEditDo extends Tv_ActionAdminOnly
{
	/**
	...
	{
		// 検証エラーの場合
		if ($this->af->validate() > 0) return 'admin_thema_edit';
		return null;
	}
	...
	function perform()
	{
		...
		*/
		...
		$timestamp = date('Y-m-d H:i:s');
		$o->set('thema_updated_time', $timestamp);
		
		// 更新
		$r = $o->update();
		// 更新エラーの場合
		if(Ethna::isError($r)) return 'admin_thema_edit';
		
		$this->ae->add(null, '', I_ADMIN_THEMA_EDIT_DONE);
