...
//			if(!$this->af->get('no_check')){
		...
		if($GLOBALS['EMOJIOBJ']->carrier=='AU'){
			...
			$this->ae->add('errors', "大変申し訳ありませんがPC広告には非対応となっております。");
			return 'user_error';
		}
		// 広告URLが空の場合
		if(!$ad_url){
			$this->ae->add('errors', "大変申し訳ありませんがこの広告はお客様のキャリアではご利用になる事ができません。");
			return 'user_error';
		}
		...
		if( $a->get('ad_stock_status') == 1 ){
			...
			if( $a->get('ad_stock_num') == 0 ){
				$this->ae->add("errors","この広告は配信を終了しました｡");
				return 'user_error';
			}
		}
		...
		if ( $a->get('ad_start_status') == 1 ) {
			if ( $a->get('ad_start_time') >= date('Y-m-d H:i:s') ) {
				$this->ae->add("errors","この広告はまだ配信を開始していません｡");
				return 'user_error';
			}
		}
...
//mail("fujimori@technovarth.co.jp", "ad redirect", $ad_url, "From:info@euphee.com");
//mail("mazda@technovarth.co.jp", "ad redirect", $ad_url, "From:info@euphee.com");
