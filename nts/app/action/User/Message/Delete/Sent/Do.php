...
/**
 * ユーザ送信済みメッセージ削除実行アクションフォーム
 * 
 * @author Technovarth
 * @access public
 * @package SNSV
 */
class Tv_Form_UserMessageDeleteSentDo extends Tv_ActionForm
{
...
/**
 * ユーザ送信済みメッセージ削除実行アクション
 * 
 * @author Technovarth
 * @access public
 * @package SNSV
 */
class Tv_Action_UserMessageDeleteSentDo extends Tv_ActionUserOnly
{
	/**
	...
	function perform()
	{
		...
		if($message->get('message_status_to') == 3){
			/* 差出人、受取人双方が削除したとき物理削除 */
			$message->remove();
		}else{
			$message->set('message_status_from', 3);
			$message->update();
		};
