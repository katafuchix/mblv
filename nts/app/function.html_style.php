...
EOD;
	// mailto
		...
		$html .= "mailto:" . $config['mail_account'][$account]['account'] . "_" . $hash . "@" . $config['mail_domain'];
		
		$html .= "?subject=" ;
		if($subject){
			$html .= $subject;
		}else{
			$html .= $config['mail_account'][$account]['subject'];
		}
		...
		if($body){
			...
			$html .= $config['mail_account'][$account]['body'];
		}
...
EOD;
	...
	if($params['type'] == 'line'){
		...
		$line_url = $config['base_url'] . 'img/' . $line;
		if($GLOBALS['MOBILETYPE'] == "softbank"){
			$html .= <<<EOD
...
EOD;
	...
	elseif($params['type'] == 'img_left'){
		$src = $params['src'];
			$html .= <<<EOD
<img src="$src" align="left" style="float:left;margin:0px 5px 0px 5px;" />
EOD;
	}
	// file
