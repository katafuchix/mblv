...
//class Tv_View_AdminStatsList extends Tv_ViewClass
{
	/**
	 * 画面表示前処理
	 * 
...
//print_r($listview_data);
		...
		if($total[view]){
			$this->af->setApp('pv', $total[view]);
		}else if($total[access]){
			$this->af->setApp('pv', $total[access]);
		}else{
			
		}
		$this->af->setApp('score_keys', $this->score_keys);
		
		// 総合計算出 listview 実行時のみ
		if($total_data){
			list($total_data, $totalAll) = $this->_calcList($total_data, $param);
			$this->af->setApp('totalAll', $totalAll);
		}
	}
