...
class Tv_View_UserAvatarList extends Tv_ListViewClass
{
	...
	function preforward()
	{
		...
		// 一般ユーザの場合は優先度「1」まで表示させる
		$priority = 1;
		// ゲストユーザの場合は優先度「-1」まで表示させる
		if($u->get('user_guest_status') == 1) $priority = -1;
		// スタッフユーザの場合は優先度「-2」まで表示させる
		if($u->get('user_guest_status') == 2) $priority = -2;
		$sqlWhere .= " AND ac.avatar_category_status = 1 AND ac.avatar_category_priority_id >= ? AND ac.avatar_category_priority_id <> 0";
		$sqlValues[] = $priority;
