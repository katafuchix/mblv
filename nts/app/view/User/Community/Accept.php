...
class Tv_View_UserCommunityAccept extends Tv_ViewClass
{
	...
	function preforward()
	{
		...
		foreach($communityList[1] as $i => $community){
			...
			if($userList[0] > 0){
				// コミュニティ名を取得
				$communityObj = &new Tv_Community($this->backend,
					'community_id',
					$community['community_id']
					);
				$communityList[1][$i]['community_title'] = $communityObj->get('community_title');
			...
			// 上の1行をifに入れていないのは承認待ちユーザの有無の判定に$user_list[0]を利用するため
		}
