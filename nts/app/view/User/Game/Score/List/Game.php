...
class Tv_View_UserGameScoreListGame extends Tv_ListViewClass
{
	...
	function preforward()
	{
		...
		$sqlWhere = 'game_id=? AND s1.user_id=u.user_id';
		...
		if($this->af->get('term') != 'all'){
			...
			$sqlWhere2 .= " AND s2.user_game_score_created_time >= '" . sprintf("%04d-%02d-%02d 00:00:00", $year, $month, 1) . "' AND s2.user_game_score_created_time <= '" . sprintf("%04d-%02d-%02d 23:59:59", $year, $month, $day) . "'";
		}
		// リストビュー
		$this->listview = array(
