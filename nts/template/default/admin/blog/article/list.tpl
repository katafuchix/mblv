		...
		<div id="mainC">
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.blog_article_status_active}class="active"{/if}>{form_input name="blog_article_status" emptyoption=""}</td>
			...
			<table class="sheet" id="list">
				...
				<tr>
					<td class="checkedrow" colspan=10 align="right">下記{$ft.blog_article.name}の{$ft.blog_article_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
				...
				<tr>
					...
					<td nowrap>
						...
						[更新]{$item.blog_article_updated_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
				...
				{/foreach}
				<tr>
					<td class="checkedrow" colspan=10 align="right">上記{$ft.blog_article.name}の{$ft.blog_article_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
			</table>
			{/form}
