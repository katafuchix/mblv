		...
		<div id="mainC">
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.message_status_active}class="active"{/if}>{form_input name="message_status" emptyoption=""}</td>
				</tr>
				<tr>
			...
			<table class="sheet" id="list">
				...
				<tr>
					<td class="checkedrow" colspan=10 align="right">下記{$ft.message.name}の{$ft.message_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
				...
				<tr>
					...
					<td nowrap>
						...
						[更新]{$item.message_updated_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
				...
				{/foreach}
				<tr>
					<td class="checkedrow" colspan=10 align="right">上記{$ft.message.name}の{$ft.message_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
			</table>
			{/form}
			{include file="admin/pager.tpl"}
