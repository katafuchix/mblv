		...
		<div id="mainC">
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.report_status_active}class="active"{/if}>{form_input name="report_status" emptyoption=""}</td>
				</tr>
				<tr>
			...
			<table class="sheet" id="list">
				...
				<tr>
					...
					<td nowrap>
						[投稿]{$item.report_regist_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
						[更新]{$item.report_update_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
						[削除]{if $item.report_status ==0}{$item.report_delete_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}{/if}
					</td>
					<td>{$item.report_user_id}</td>
					<td><a href="?action_admin_user_edit=true&user_id={$item.report_user_id}">{$item.report_user_nickname}</a>さん</td>
					<td>{$item.report_fail_user_id}</td>
					<td><a href="?action_admin_user_edit=true&user_id={$item.report_fail_user_id}">{$item.report_fail_user_nickname}</a>さん</td>
