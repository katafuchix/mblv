...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			<!-- ここからメインコンテンツ-->
			<h2>{$ft.thema.name}編集</h2>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
			</div>
			{form ethna_action="admin_thema_edit_do"}
			{form_input name="thema_id"}
			<table class="sheet" id="w200">
				<tr>
					<th>{form_name name="thema_id"}</th>
					<td>{$form.thema_id}</td>
				</tr>
				<tr>
					<th>{form_name name="thema_title"}</th>
					<td>{form_input name="thema_title" size="50"}</td>
				</tr>
				<tr>
					<th>{form_name name="thema_term"}</th>
					<td>
						{form_input name="thema_term_status"}{form_name name="thema_term_status"}<br />
						{form_input name="thema_term"}時間ごと
					</td>
				</tr>
			<!--
				<tr>
