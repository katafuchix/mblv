...
<div align="left" style="text-align:left;font-size:small;">
	...
	{form ethna_action="user_community_comment_edit_confirm" action="$script"}
		...
		{if $form.image_id}
			...
			<span>
			<a href="{html_style type='mailto' account='community_comment_image' hash=$form.community_comment_hash}">{$ft.image_edit.name}</a><br />
			{form_input name="delete_image"}
			</span>
			{html_style type="br"}
		{else}
			<div style="text-align:center;font-size:small;">
			<a href="{html_style type='mailto' account='community_comment_image' hash=$form.community_comment_hash}">{$ft.image_add.name}</a><br />
			</div>
		{/if}
