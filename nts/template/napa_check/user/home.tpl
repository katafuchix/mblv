...
<!--タイトル-->
{html_style type="title" title="`$session.user.user_nickname`さんのマイページ" bgcolor=$title_bgcolor fontcolor=$title_fontcolor}
...
<div align="left" style="text-align:left;font-size:small;">
	...
	<td>
		<div style="float:left;font-size:small;">
			...
			<a href="?action_user_blog_article_add=true">{$ft.blog_article.name}を書く</a><br />
			<a href="?action_user_blog_view=true&user_id={$session.user.user_id}">{$ft.blog_article.name}を読む</a><br />
			<a href="?action_user_community_list=true">{$ft.community.name}{if $app.community_count > 0}({$app.community_count}){/if}</a><br />
			<a href="?action_user_footprint=true">あしあと</a><br />
			{if $config.option.avatar && !$session.user.avatar_config_first}
			...
			<a href="?action_user_point_home=true">ﾅﾊﾟﾎﾟ通帳</a><br />
...
<!--タイトル-->
{html_style type="title" title="管理ﾒﾆｭｰ" bgcolor=$title_bgcolor fontcolor=$title_fontcolor}

<!--下部メニュー開始-->
<div align="left" style="text-align:left;font-size:small;">
	<span style="color:#ffcc00">■</span><a href="?action_user_profile_view=true&user_id={$session.user.user_id}">{$ft.profile.name}</a><br />
	<span style="color:#ffcc00">■</span><a href="?action_user_config=true">設定</a><br />
	<span style="color:#ffcc00">■</span><a href="?action_user_invite=true">友達招待</a><br />
	<span style="color:#ffcc00">■</span><a href="?action_user_blacklist_list=true">{$ft.blacklist.name}管理</a><br />
	<span style="color:#ffcc00">■</span><a href="fp.php?code=guide_point">{$ft.point.name}の集め方</a><br />
	<span style="color:#ffcc00">■</span><a href="fp.php?code=guide_inquiry">問い合わせ</a><br />
	<span style="color:#ffcc00">■</span><a href="?action_user_logout_do=true">ﾛｸﾞｱｳﾄ</a><br />
	<span style="color:#ffcc00">■</span><a href="?action_user_resign_confirm=true">退会</a><br />
</div>
<!--下部メニュー終了-->
