...
<div align="left" style="text-align:left;font-size:small;">
	...
	{foreach from=$app.listview_data item=item name=ad key =k}
		{if $k%2==0}
			...
			[{$config.point_type[$item.point_type].name}]{$item.point_memo}&nbsp;&nbsp;{$item.point}{$ft.point.name}({$config.point_status[$item.point_status].name})</a>
			{/style}
		{else}
	...
	{/foreach}
	{include file="user/pager.tpl"}
	<span style="color:{$menucolor}">{$ft.menu_icon.name}</span><a href="?action_user_ad=true">{$ft.point.name}GETへ</a><br />
</div>
<!--コンテンツ終了-->
