...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			...
			<blockquote><a href="?action_admin_util=true&page=faq_bbs_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">{$ft.bbs.name}管理FAQ</a></blockquote>
			<div class="entry_box">
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.bbs_status_active}class="active"{/if}>{form_input name="bbs_status"}</td>
				</tr>
				<tr>
					<th></th>
					<td></td>
			...
			<table class="sheet" id="list">
				<tr>
					...
					<th width=220>{$ft.bbs_body.name}</th>
					<th>{$ft.image.name}</th>
					<th nowrap><input type="checkbox" onclick="$j('#list input:checkbox').attr('checked', this.checked);">非表示</th>
				</tr>
				<tr>
					<td class="checkedrow" colspan=11 align="right">下記{$ft.bbs.name}の{$ft.bbs_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
				...
				<tr>
					...
					<td nowrap>
						...
						[更新]{$item.bbs_updted_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
					...
					<td width=220>{$item.bbs_body|nl2br}</td>
				...
				{/foreach}
				<tr>
					<td class="checkedrow" colspan=11 align="right">上記{$ft.bbs.name}の{$ft.bbs_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
			</table>
			{/form}
