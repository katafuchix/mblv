...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			...
			<blockquote><a href="?action_admin_util=true&page=faq_blacklist_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">{$ft.blacklist.name}検索FAQ</a></blockquote>
			<div class="entry_box">
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.black_list_status_active}class="active"{/if}>{form_input name="black_list_status"}</td>
			...
			<table class="sheet">
				<tr>
					<th>{$ft.black_list_id.name}</th>
					<th>{$ft.black_list_status.name}</th>
					<th>{$ft.black_list_checked.name}</th>
					<th>
						登録日時<br />
						更新日時<br />
						削除日時
					</th>
					<th colspan="2">{$ft.black_list_user_id.name}と{$ft.user_nickname.name}</th>
					<th colspan="2">{$ft.black_list_deny_user_id.name}と{$ft.user_nickname.name}</th>
				...
				<tr>
					...
					<td nowrap>
						[投稿]{$item.black_list_created_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
						[更新]{$item.black_list_updted_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
						[削除]{if $item.black_list_status == 0}
								{$item.black_list_deleted_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}
							  {/if}
					</td>
