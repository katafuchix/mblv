...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			<!-- ここからメインコンテンツ-->
			<h2>{$ft.blog_article.name}{$ft.blog_comment.name}検索</h2>
			<blockquote><a href="?action_admin_util=true&page=faq_blog_comment_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">{$ft.blog_article.name}{$ft.blog_comment.name}検索FAQ</a></blockquote>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
				{if $app.listview_total == 0}
					検索条件に合致する{$ft.blog_article.name}{$ft.blog_comment.name}は見つかりませんでした。
				{else}
					検索条件に合致する{$ft.blog_article.name}{$ft.blog_comment.name}が{$app.listview_total}件見つかりました。
				{/if}
			</div>
			...
			<table class="sheet" id="list">
				...
				<tr>
					<td class="checkedrow" colspan=11 align="right">下記{$ft.blog_comment.name}の{$ft.blog_comment_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
				...
				<tr>
					...
					<td nowrap>
						...
						[更新]{$item.blog_comment_updted_time|jp_date_format:"%Y/%m/%d(%a) %H:%M"}<br />
				...
				{/foreach}
				<tr>
					<td class="checkedrow" colspan=11 align="right">上記{$ft.blog_comment.name}の{$ft.blog_comment_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
			</table>
			{/form}
