...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			...
			<blockquote><a href="?action_admin_util=true&page=faq_community_article_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">{$ft.community_article.name}検索FAQ</a></blockquote>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
				{if $app.listview_total == 0}
					検索条件に合致する{$ft.community_article.name}は見つかりませんでした。
				{else}
					検索条件に合致する{$ft.community_article.name}が{$app.listview_total}件見つかりました。
				{/if}
			</div>
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.community_article_status_active}class="active"{/if}>{form_input name="community_article_status"}</td>
				</tr>
				<tr>
			...
			<table class="sheet">
				...
				<tr>
					<td class="checkedrow" colspan=11 align="right">下記{$ft.community_article.name}の{$ft.community_article_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
				...
				{/foreach}
				<tr>
					<td class="checkedrow" colspan=11 align="right">上記{$ft.community_article.name}の{$ft.community_article_checked.name}ステータスを更新する{form_submit name="update" value="　実　行　"}</td>
				</tr>
			</table>
			{/form}
