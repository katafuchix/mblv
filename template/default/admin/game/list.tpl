...
<div id="two_column">
	...
	<div id="main" class="floatr">
		...
		<!-- ここからメインコンテンツ-->
			<h2>
			{if $app.game_category_name}
				{$app.game_category_name}	>
			{/if}
				{$ft.game.name}管理
			</h2>
			<blockquote><a href="?action_admin_util=true&page=faq_game_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">{$ft.game.name}管理FAQ</a></blockquote>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
				{$ft.menu_icon.name}<a href="?action_admin_config_edit=true&site_type=game">{$ft.game.name}基本設定</a>
				{$ft.menu_icon.name}<a href="?action_admin_gamecategory_list=true">{$ft.game_category.name}管理</a>
				{$ft.menu_icon.name}<a href="?action_admin_game_add=true">{$ft.game.name}登録</a><br />
				
				{if $app.listview_total == 0}
					検索条件に合致する{$ft.game.name}は見つかりませんでした。
				{else}
					検索条件に合致する{$ft.game.name}が{$app.listview_total}件見つかりました。
				{/if}
			</div>
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.game_status_active}class="active"{/if}>{form_input name="game_status"}</td>
				</tr>
				<tr>
					<th></th>
					<td></td>
