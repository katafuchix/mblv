...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			<!-- ここからメインコンテンツ-->
			<h2>{$ft.magazine.name}編集</h2>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
			</div>
			{form name="form00" action="$script" method="post" ethna_action="admin_magazine_edit_do" onsubmit="updateTextArea('magazine_body_html')"}
...
generate_wysiwyg('magazine_body_html');
				...
				{if $form.magazine_status==1}
				<tr>
					<td></td>
					<td>{form_submit value="　`$ft.magazine.name`編集　"}</td>
				</tr>
				{/if}
			</table>
			{/form}
