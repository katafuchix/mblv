...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			<!-- ここからメインコンテンツ-->
			<h2>{$ft.magazine.name}カレンダー</h2>
			<blockquote><a href="?action_admin_util=true&page=faq_magazine_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">メルマガリストFAQ</a></blockquote>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
				
				{$ft.menu_icon.name}<a href="?action_admin_magazine_add=true">{$ft.magazine.name}登録</a><br />
