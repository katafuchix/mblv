...
<div id="two_column">
	...
	<div id="main" class="floatr">
		<div id="mainC">
			<!-- ここからメインコンテンツ-->
			<h2>{$ft.segment.name}編集</h2>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
			</div>
			{form action="$script" ethna_action="admin_segment_edit_do"}
			{form_input name="segment_id"}
			<table class="sheet">
				<tr>
					<th>{form_name name="segment_name"}<br /><span class="required"></span></th>
				...
				<tr>
					...
					<td>
						{form_input name="user_point_min" size="8"}pt〜{form_input name="user_point_max" size="8"}pt
					</td>
				</tr>
				<tr>
					<th>
				...
				<tr>
					<th>
						{form_name name="item_id_status"}：<br />
						{form_input name="item_id_status"}
					</th>
					<td>
				...
				<tr>
					<th></th>
					<td>
						{form_submit value="`$ft.segment.name`編集"}
					</td>
				</tr>
			</table>
			{/form}
