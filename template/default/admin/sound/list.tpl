...
<div id="two_column">
	...
	<div id="main" class="floatr">
		...
		<!-- ここからメインコンテンツ-->
			<h2>
			{if $app.sound_category_name}
				{$app.sound_category_name}	>
			{/if}
				{$ft.sound.name}管理
			</h2>
			<blockquote><a href="?action_admin_util=true&page=faq_sound_list&keepThis=true&TB_iframe=true&height=700&width=700" class="thickbox">{$ft.sound.name}管理FAQ</a></blockquote>
			<div class="entry_box">
				{if count($errors)}<span class="err">{foreach from=$errors item=error}{$error}<br />{/foreach}</span>{/if}
				{$ft.menu_icon.name}<a href="?action_admin_config_edit=true&site_type=sound">{$ft.sound.name}基本設定</a>
				{$ft.menu_icon.name}<a href="?action_admin_soundcategory_list=true">{$ft.sound_category.name}管理</a>
				{$ft.menu_icon.name}<a href="?action_admin_sound_add=true">{$ft.sound.name}登録</a><br />
				
				{if $app.listview_total == 0}
					検索条件に合致する{$ft.sound.name}は見つかりませんでした。
				{else}
					検索条件に合致する{$ft.sound.name}が{$app.listview_total}件見つかりました。
				{/if}
			</div>
			...
			<table class="sheet">
				...
				<tr>
					...
					<td {if $app.sound_status_active}class="active"{/if}>{form_input name="sound_status"}</td>
				</tr>
				<tr>
					<th></th>
					<td></td>
