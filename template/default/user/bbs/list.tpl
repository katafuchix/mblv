...
<div align="left" style="text-align:left;font-size:small;">
	...
	{foreach from=$app.listview_data item=item}
		<div align="left" style="text-align:left;float:left;">
			...
			<span>
				...
				{if $form.user_id == $session.user.user_id && $item.bbs_notice==1}<span style="color:#FF0000">{/if}
				{$item.bbs_body}
				{if $form.user_id == $session.user.user_id && $item.bbs_notice==1}</span>{/if}
				{if $item.image_id}
				<a href="?action_user_file_image_view=true&image_id={$item.image_id}&bbs_id={$item.image_id}&to_user_id={$form.user_id}"><img src="f.php?type=image&id={$item.image_id}&attr=i&SESSID={$SESSID}" alt="画像"></a>
				{/if}
			</span>
		</div>
	...
	<br>
	{include file="user/pager.tpl"}
	<span style="color:{$menucolor}">{$ft.menu_icon.name}</span><a href="?action_user_profile_view=true&user_id={$app.user.user_id}">{$app.user.user_nickname}さんの{$ft.profile.name}</a>
</div>
<!--コンテンツ終了-->
