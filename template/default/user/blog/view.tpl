...
<div align="left" style="text-align:left;font-size:small;">
	...
	{foreach from=$app.article_list item=item key=k}
		...
		{/if}
		<br />
		<a href="?action_user_blog_article_view=true&blog_article_id={$item.blog_article_id}">{$item.blog_article_title}</a>
		({$item.blog_article_comment_num})
		{if $form.user_id == $session.user.user_id && $item.blog_article_notice > 0}&nbsp;[x:0112]{/if}
		<br />
