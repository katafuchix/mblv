...
<div align="left" style="text-align:left;font-size:small;">
	...
	{form action="$script" ethna_action='user_community_admin_power_give_do'}
		...
		<br />
		<div align="left" style="text-align:left;font-size:small;">
			<a href="?action_user_profile_view=true&user_id={$app.new_admin_user.user_id}">{$app.new_admin_user.user_nickname}</a>さんに管理権を渡しますか？<br />
			<br />
		</div>
		<div style="text-align:center;font-size:small;">{form_input name="give"}<br />{form_input name="back"}</div>
	{/form}
	<hr color="{$hrcolor}" style="border:solid 1px {$hrcolor};">
	<span style="color:{$menucolor}">{$ft.menu_icon.name}</span><a href="?action_user_community_view=true&community_id={$app.community_id}">{$ft.community.name}へ戻る</a>
</div>
<!--コンテンツ終了-->
